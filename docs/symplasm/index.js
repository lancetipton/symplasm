...
function lex(state) {
  const { str, options: { childlessTags: childlessTags2 } } = state;
...
function parse(state) {
  ...
  while (cursor < len) {
    ...
    const hasChildren = !(attrToken.close || arrayIncludes(options3.voidTags, tagName));
    if (hasChildren) {
...
var revPropMap = Object.entries(propMap).reduce((obj, [key, value]) => ({ ...obj, [value]: key }), {});
    ...
    if (!value)
      ...
      Object.keys(value).map((_name) => {
        ...
        if (options3.hasOpts && options3.styleAsCss)
          name = _name.split(/(?=[A-Z])/).join("-").toLowerCase();
...
var buildTag = (tagName, attributes, children, options3) => {
  return `<${tagName}${formatAttributes(attributes, options3)}>${toHTML(children, options3) || ""}</${tagName}>`;
};
...
var convertBlock = (block, parent, nodes, children, tree) => {
  const data = selectorCheck.tagConvert[block[0]] ? runAction({
  ...
  block[1] = typeof block[1] === "object" ? Object.keys(block[1]).reduce((attrs, key) => {
    let useKey = selectorCheck.attrKeyConvert[key] ? runAction({
    ...
    if (useKey && block[1][key]) {
      attrs[useKey] = selectorCheck.attrValueConvert[key] ? runAction({
        ...
        nodes,
        children
      }, "value") : typeof block[1][key] === "string" ? unquote(block[1][key]) : block[1][key];
    }
    return attrs;
  }, {}) : {};
...
var format = ({ parent, childs, nodes, tree, ...args }) => {
  return childs ? childs.reduce((children, node) => {
    nodes = nodes || childs;
    const child = node.type === "text" || node.type === "comment" ? filterFS(node, parent) : formatNode({
      node,
      childs,
      nodes,
      children,
...
var formatNode = ({ node, nodes, children, tree, parent }) => {
  const block = selectorCheck.tagConvert[node[0]] ? tagConvert({
...
var formatAttributes2 = (args) => {
  ...
  Object.keys(attributes).map((item) => {
    ...
    if (selectorCheck.attrKeyConvert[parts[0]] === null)
      return;
    let key = selectorCheck.attrKeyConvert[parts[0]] ? runAction({
...
var formatValue = (args) => {
  ...
  const updatedVal = key === "style" && typeof value === "string" ? convertStyle(unquote(value)) : selectorCheck.attrValueConvert[key] ? runAction({
    action: selectorCheck.attrValueConvert[key],
    value: unquote(value),
    node,
    key,
  ...
  switch (typeof action) {
    ...
    case "function":
      return action({
        ...
        1: node[1],
        2: node[2]
      }, key, value, nodes, children, options2) || args[def];
    case "object":
      let updateVal;
...
var allElementsCB = (block, parent, tree) => {
  return typeof options2.allElements === "function" ? options2.allElements(block, parent, tree) || block : block;
};
var formatFS = (nodes, _options) => {
  ...
  rootFS[2] = Array.isArray(rootFS[2]) ? rootFS[2].map((child) => {
